...
function createCourseCard(course) {
  ...
  code.textContent = `${course.completed ? "✓ " : ""}${course.subject} ${course.number}`;
  ...
  card.append(code, title, credits, status);
  return card;
}

function displayCourses(courseSelection) {
  const cards = courseSelection.map(createCourseCard);
  courseList.replaceChildren(...cards);

  const creditSum = courseSelection.reduce((total, course) => total + course.credits, 0);
...
filterButtons.forEach((button) => {
  button.addEventListener("click", () => {
    ...
    const filteredCourses =
      selectedFilter === "all"
        ? courses
        : courses.filter((course) => course.subject === selectedFilter);
